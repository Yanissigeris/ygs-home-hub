...
const GuideOfferBlockEn = ({ guideType, title, subtitle, ctaLabel, background = "default" }: Props) => {
  const [open, setOpen] = useState(false);
  const label = ctaLabel ?? defaultLabels[guideType] ?? "Get the guide";
  ...
  return (
    <>
      <motion.div
        ...
        className={`flex flex-col h-full overflow-hidden rounded-[var(--card-radius)] border border-border p-5 sm:p-6 shadow-[var(--card-shadow)] hover:shadow-[var(--card-shadow-hover)] transition-shadow duration-300 ${background === "alt" ? "bg-secondary/30" : "bg-card"}`}
      >
        ...
        <Button variant="accent" size="default" className="mt-5 w-full sm:w-auto self-start text-[0.8125rem] max-w-full" onClick={() => setOpen(true)}>
...
export default GuideOfferBlockEn;
