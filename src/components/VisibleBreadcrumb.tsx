...
/** Route → breadcrumb trail. Mirrors BreadcrumbJsonLd.tsx — single source of truth. */
...
const VisibleBreadcrumb = () => {
  ...
  // No breadcrumb on homepage or unmapped routes
  if (!config) return null;

  const isHome = (name: string) => name === "Accueil" || name === "Home";

  return (
    <nav aria-label="Breadcrumb" className="bg-secondary/40 border-b border-border/40">
      <div className="section-container">
        <ol className="flex items-center gap-1.5 py-2.5 text-[0.8125rem] leading-none overflow-x-auto">
          {config.trail.map((crumb, i) => (
            <React.Fragment key={crumb.href}>
              <li className="flex items-center gap-1.5 shrink-0">
                ...
                >
                  {i === 0 && isHome(crumb.name) && <Home size={13} className="shrink-0" />}
          ...
          <li className="shrink-0 font-medium text-foreground" aria-current="page">
            {config.current}
          </li>
        </ol>
      </div>
